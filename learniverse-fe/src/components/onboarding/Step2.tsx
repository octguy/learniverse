...
export default function Step2({ onNext, onPrev, selectedValue, onChange }: Step2Props) {
  return (
      <div className="flex flex-col justify-between bg-gradient-to-b from-[#f6f6ff] to-white w-full max-w-[1250px] h-full min-h-[60vh] lg:min-h-[50vh] xl:min-h-[45vh] 2xl:min-h-[40vh] rounded-xl p-10 relative">
        ...
        <div className="flex justify-end gap-4 mt-10">
          ...
          <Button className={cn("bg-blue-500 hover:bg-blue-600 text-white rounded px-4 py-2", !selectedValue && "opacity-50 cursor-not-allowed")} disabled={!selectedValue} onClick={onNext}>Tiếp tục →</Button>
        </div>
      </div>
  )
}